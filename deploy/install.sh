...
check_ports_occupied() {
    ...
    if [[ -n $port_check_output ]]; then
        ...
        echo "+++++++++++ ERROR ++++++++++++++++++++++"
        echo "SigNoz requires ports 80 & 4317 to be open. Please shut down any other service(s) that may be running on these ports."
...
wait_for_containers_start() {
    ...
    while [[ $timeout -gt 0 ]]; do
        status_code="$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:80/api/v1/health?live=1" || true)"
...
if [[ $status_code -ne 200 ]]; then
    ...
    echo ""
    echo -e "🟢 Your frontend is running on http://localhost:80"
    echo ""
